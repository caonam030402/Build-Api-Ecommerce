...
exports.promotionService = {
                    ...
                    price: {
                        $cond: {
                            ...
                            then: '$price',
                                ...
                                $function: {
                                    body: utils_1.generalPricePromotion.toString(),
                                    args: ['$price'],
                                    lang: 'js'
                                }
                            }
    ...
    createPromotions: async (promotionArray) => {
        ...
        for (const promotionBody of promotionArray) {
            ...
            if (promotion) {
                throw new errorHandlers_1.ApiError('Sản phẩm trong khung giờ bị trùng', http_status_1.default.UNPROCESSABLE_ENTITY, '');
            }
            const newPromotion = await promotionModel_1.Promotion.create(promotionBody);
            newPromotions.push(newPromotion);
            await productModel_1.Product.findByIdAndUpdate(promotionBody.product, { promotion: newPromotion._id }, { new: true });
        }
